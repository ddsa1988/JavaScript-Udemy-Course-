...
function calcAverageHumanAge(dogsAge) {
    if (Array.isArray(dogsAge)) {
        const humanAges = dogsAge.map((value) =>
            value <= 2 ? value * 2 : value * 4 + 16
        );
        console.log(`Human Ages: [${humanAges}]`);

        const humanAdults = humanAges.filter((value) => value >= 18);
        console.log(`Adults: [${humanAdults}]`);

        const averageHumanAges = humanAdults.reduce(
